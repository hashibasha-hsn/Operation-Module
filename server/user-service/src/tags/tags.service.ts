...
export class TagsService {
  ...
  async updateAdvDropdownTag(
    id: string,
    tagData: Partial<AdvDropdownTag & { values?: Array<{ value: string } | string> }>,
  ): Promise<AdvDropdownTag> {
    const tag = await this.findOneAdvDropdownTag(id);
    if (!tag) {
      throw new NotFoundException(`Adv dropdown tag ${id} not found`);
    }

    const { values, ...rest } = tagData;
    if (rest.rootTagName !== undefined) {
      tag.rootTagName = rest.rootTagName;
    }
    if (rest.isActive !== undefined) {
      tag.isActive = rest.isActive;
    }

    if (values !== undefined) {
      await this.advDropdownValueRepository.delete({ tagId: id });
      tag.values = values
        .map((entry) => (typeof entry === 'string' ? entry : entry?.value)?.trim())
        .filter(Boolean)
        .map((value) =>
          this.advDropdownValueRepository.create({
            value,
            tagId: id,
          }),
        );
