...
export async function fetchConversations(organizationId = getOrganizationId()): Promise<ChatConversation[]> {
  try {
    ...
    const response = await fetch(`${ORG_API}/communication/conversations?${params}`);
...
export async function createDirectConversation(
  ...
  const response = await fetch(`${ORG_API}/communication/conversations/direct`, {
    method: "POST",
...
export async function createChannel(
  ...
  const response = await fetch(`${ORG_API}/communication/conversations/channel`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function markConversationRead(conversationId: string): Promise<void> {
  await fetch(`${ORG_API}/communication/conversations/${conversationId}/read`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function addChannelMembers(conversationId: string, userIds: string[]): Promise<ChatConversation> {
  const response = await fetch(`${ORG_API}/communication/conversations/${conversationId}/members`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function removeChannelMember(conversationId: string, userId: string): Promise<ChatConversation> {
  ...
  const response = await fetch(
    `${ORG_API}/communication/conversations/${conversationId}/members/${encodeURIComponent(userId)}?${params}`,
    { method: "DELETE" },
  );
  return handle<ChatConversation>(response);
}
...
export async function setNotificationPreference(
  ...
  await fetch(`${ORG_API}/communication/conversations/${conversationId}/notification`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function fetchMessages(
  ...
  const response = await fetch(`${ORG_API}/communication/conversations/${conversationId}/messages?${params}`);
  if (!response.ok) return [];
  const data = await response.json();
  return Array.isArray(data) ? data : [];
}
...
export async function sendMessage(
  ...
  const response = await fetch(`${ORG_API}/communication/conversations/${conversationId}/messages`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ meId: currentUser(), body, attachment: attachment ?? null }),
  });
...
export async function editMessage(
  ...
  const response = await fetch(
    `${ORG_API}/communication/conversations/${conversationId}/messages/${messageId}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
export async function deleteMessage(conversationId: string, messageId: string): Promise<void> {
  const params = new URLSearchParams({ meId: currentUser() });
  await fetch(
    `${ORG_API}/communication/conversations/${conversationId}/messages/${messageId}?${params}`,
    { method: "DELETE" },
  );
}
...
export async function uploadChatFile(file: File): Promise<ChatAttachment> {
  ...
  const response = await fetch(`${ORG_API}/communication/upload`, {
    method: "POST",
    body: form,
  });
