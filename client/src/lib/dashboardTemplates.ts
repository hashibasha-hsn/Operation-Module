...
export function templatesForTab(tab: string): DashboardTemplateDefinition[] {
  if (tab === 'Ticket & Action Point') {
    return DASHBOARD_TEMPLATE_LIBRARY.filter(
      (t) => t.category === 'ticket' || t.category === 'action-point',
    );
  }
  return DASHBOARD_TEMPLATE_LIBRARY.filter((t) => t.category === 'process-workflow');
}
