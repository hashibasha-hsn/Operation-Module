...
function getContext() {
  ...
  return {
    ...
    userName: (user.fullName || user.name || user.email || 'User') as string,
  };
}
...
export async function updateTicketStatus(
  id: string,
  status: TicketStatus,
  options?: { closureAnswers?: Record<string, unknown> },
) {
  ...
  return ticketRequest<TicketRecord>(`/tickets/${id}/status`, {
    ...
    body: JSON.stringify({
      status,
      userId,
      closureAnswers: options?.closureAnswers,
    }),
  });
}
...
export async function fetchTicketClosureQuestions(activeOnly = false) {
  const { organizationId } = getContext();
  const params = new URLSearchParams({ organizationId });
  if (activeOnly) params.set('activeOnly', 'true');
  return ticketRequest<any[]>(`/tickets/closure-questions?${params}`);
}

export async function createTicketClosureQuestion(payload: Record<string, unknown>) {
  const { organizationId } = getContext();
  return ticketRequest<any>('/tickets/closure-questions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ...payload, organizationId }),
  });
...
export async function updateTicketClosureQuestion(
  id: string,
  payload: Record<string, unknown>,
) {
  return ticketRequest<any>(`/tickets/closure-questions/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
...
export async function deleteTicketClosureQuestion(id: string) {
  return ticketRequest<void>(`/tickets/closure-questions/${id}`, { method: 'DELETE' });
}

export async function applyTicketClosureRules() {
  const { organizationId } = getContext();
  return ticketRequest<{ closedCount: number }>('/tickets/rules/apply', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchTicketOrgReportFiltered(
  options: {
    startDate?: string;
    endDate?: string;
    status?: string;
