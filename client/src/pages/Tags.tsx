...
export default function Tags() {
  ...
  const [editingAdvTag, setEditingAdvTag] = useState<any>(null);
  ...
  const handleCreateAdvTag = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setEditingAdvTag(null);
        setIsAdvTagDialogOpen(false);
        fetchAdvDropdownTags();
      } else {
        console.error('Failed to save adv dropdown tag');
        const errorData = await response.json();
        console.error('Error details:', errorData);
      }
    } catch (err) {
      console.error('Error saving adv dropdown tag:', err);
    }
  };

  const handleEditAdvTag = (tag: any) => {
    setEditingAdvTag(tag);
    setAdvTagData({
      rootTagName: tag.rootTagName || "",
      values: (tag.values || []).map((entry: any) =>
        typeof entry === "string" ? entry : entry.value || "",
      ),
    });
    setIsAdvTagDialogOpen(true);
  };

  const resetAdvTagForm = () => {
    setAdvTagData({ rootTagName: "", values: [] });
    setEditingAdvTag(null);
  };
      ...
      {activeTab === t('advDropdownTag') ? (
        <>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <Dialog
              open={isAdvTagDialogOpen}
              onOpenChange={(open) => {
                setIsAdvTagDialogOpen(open);
                if (!open) resetAdvTagForm();
              }}
            >
              <DialogTrigger asChild>
                <Button className="gap-2" onClick={resetAdvTagForm}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  <DialogTitle>
                    {editingAdvTag ? t('editTag') : t('createAdvancedDropdownTag')}
                  </DialogTitle>
                  <DialogDescription>
                    {editingAdvTag
                      ? t('updateDropdownTagWithMultipleValues') || 'Update dropdown tag and its values'
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleCreateAdvTag}>
                    {editingAdvTag ? t('save') : t('createTag')}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
                  ...
                  {advDropdownTags.length === 0 ? (
                    ...
                    advDropdownTags
                      ...
                      <TableRow key={tag.id}>
                        ...
                        <TableCell>
                          <TableActionsMenu>
                            <DropdownMenuItem onClick={() => handleEditAdvTag(tag)}>
                              {t('edit')}
                            </DropdownMenuItem>
