...
export default function ProcessFill() {
      ...
      .then(([processData, entityData, userSubmissions]) => {
        ...
        const existingDraft =
          userSubmissions.find(
            (item: any) =>
              item.workflowId === processId && item.status === "correction",
          ) ||
          userSubmissions.find(
            (item: any) => item.workflowId === processId && item.status === "draft",
          );
  ...
  const handleAnswerChange = (question: any, value: string) => {
    ...
    const additions: Record<string, string> = { [question.id]: value };
    if (config.timestamp && value) {
      additions[`${question.id}:timestamp`] = new Date().toISOString();
    }
    setResponses((prev) => ({ ...prev, ...additions }));
  ...
  const handleAddonChange = (question: any, key: string, value: string) => {
    setResponses((prev) => ({ ...prev, [`${question.id}:${key}`]: value }));
  };

  const renderQuestionAddons = (question: any) => {
    const config = question.options ?? {};
    if (
      !config.attachFile &&
      !config.markNA &&
    ...
    const commentValue = responses[`${question.id}:comment`] ?? "";
    const attachmentName = responses[`${question.id}:attachment`] ?? "";

    return (
      <div className="space-y-3 pt-1">
        {config.attachFile && config.questionReferenceFile && (
        ...
        {(config.instructionText || config.instructionAttachment) && (
          <div className="rounded-md border bg-blue-50/50 px-3 py-2 text-sm">
        ...
        {config.questionTag && (
          <div className="text-xs text-muted-foreground">
            Tag: <span className="font-medium text-orange-700">{config.questionTag}</span>
          </div>
        )}
        ...
        {config.markNA && (
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <Checkbox
              checked={naValue === "true"}
              onCheckedChange={(checked) =>
                handleAddonChange(question, "na", checked ? "true" : "false")
              }
            />
            Mark as N/A
          </label>
        ...
        {config.timestamp && (
          <div className="text-sm text-muted-foreground">
            ...
            Timestamp recorded:{" "}
            {responses[`${question.id}:timestamp`]
              ? new Date(responses[`${question.id}:timestamp`]).toLocaleString()
              : "—"}
          </div>
        )}
        ...
        {config.allowComment && (
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">
              Comment{config.commentRequired ? " (required)" : ""}
            </Label>
            <Textarea
              rows={2}
              value={commentValue}
              onChange={(e) => handleAddonChange(question, "comment", e.target.value)}
            ...
            <>
              ...
              <div className="space-y-4">
                {questions.map((question: any, index: number) => {
                  ...
                  const hasAnswer =
                    Boolean(responses[question.id]) || responses[`${question.id}:na`] === "true";
                  return (
