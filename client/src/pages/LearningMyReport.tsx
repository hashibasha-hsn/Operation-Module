...
import { fetchLearningMyReport as fetchLearningMyReportApi } from "@/lib/reportApi";
...
export default function LearningMyReport() {
  ...
  const fetchLearningMyReport = async () => {
    setLoading(true);
    try {
      const result = await fetchLearningMyReportApi({
                    ...
                    <div key={user.userId} className="flex items-center gap-3 p-2 rounded hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium">
                          {humanLabel(user.name, user.userName, user.email, "—")}
                        </p>
