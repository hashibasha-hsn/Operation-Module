...
export default function CreateForm() {
  ...
  useLayoutEffect(() => {
    if (!isAssessmentMode || !processDraft) return;
    const syncedSections = activeSectionId
      ? (syncActiveSectionQuestions(
          sections as ProcessSectionDraft[],
          activeSectionId,
          formElements,
          true,
          true,
        ) as AssessmentSectionDraft[])
      : (sections as AssessmentSectionDraft[]);
    saveAssessmentDraftLocalOnly({
      ...(processDraft as AssessmentDraftState),
      title: fieldValues.field1 || (processDraft as AssessmentDraftState).title,
  ...
  useEffect(() => {
    if (!isSectionDraftMode || !processDraft || isAssessmentMode) return;
  ...
  const handlePublish = () => {
    if (isAssessmentMode) {
      ...
      if (processDraft) {
        saveAssessmentDraftLocalOnly({
          ...(processDraft as AssessmentDraftState),
          title: fieldValues.field1 || (processDraft as AssessmentDraftState).title || "New Assessment",
          description: fieldValues.field2,
          sections: syncedSections,
        });
      }
                ...
                <div className="space-y-4">
                  {isAssessmentMode ? (
                    <Input
                      ...
                      value={fieldValues.field1}
                      onChange={(e) =>
                        setFieldValues((prev) => ({ ...prev, field1: e.target.value }))
                      }
                    />
                  ) : (
              ...
              <Tabs defaultValue="add-type" className="w-full h-full flex flex-col">
                ...
                <TabsContent value="add-type" className="flex-1 overflow-y-auto">
                  <div className="p-4 space-y-2">
                    {isAssessmentMode ? (
                      <>
                        <button type="button" className="w-full p-3 border rounded hover:bg-muted cursor-pointer flex items-center gap-2 text-left" onClick={() => addFormElement('single-answer', 'Single answer')}>
                          <Type className="w-4 h-4" />
                          <span className="text-sm">Single answer</span>
                        </button>
                        <button type="button" className="w-full p-3 border rounded hover:bg-muted cursor-pointer flex items-center gap-2 text-left" onClick={() => addFormElement('multiple-answers', 'Multiple answers')}>
                          <Type className="w-4 h-4" />
                          <span className="text-sm">Multiple answers</span>
                        </button>
                        <button type="button" className="w-full p-3 border rounded hover:bg-muted cursor-pointer flex items-center gap-2 text-left" onClick={() => addFormElement('short-answer', 'Short answer')}>
                          <Type className="w-4 h-4" />
                          <span className="text-sm">Short answer</span>
                        </button>
                        <button type="button" className="w-full p-3 border rounded hover:bg-muted cursor-pointer flex items-center gap-2 text-left" onClick={() => addSection()}>
                          <Type className="w-4 h-4" />
                          <span className="text-sm">Section</span>
                        </button>
                        <button type="button" className="w-full p-3 border rounded hover:bg-muted cursor-pointer flex items-center gap-2 text-left" onClick={() => addSection('Sub-section')}>
                          <Type className="w-4 h-4" />
