...
export default function AssessmentCreation() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchEntities(), fetchDesignations(), fetchAssigneeProfiles()])
      .then(([entityRows, designationRows, profileRows]) => {
        setStores(
        ...
        setProfiles(
          profileRows.map((row: any) => ({
            id: row.id,
            label: row.profileName || row.name || row.id,
          })),
        );
      })
      .catch(() => toast.error(t("couldNotLoadUsersOrStores") || "Could not load assignment options"));
    ...
    if (loaded.title?.trim()) {
      ensureAssessmentDraftSaved(loaded)
        ...
        .catch((error: any) => toast.error(error.message || "Could not sync assessment draft"));
    }
  }, [t]);
