...
export default function AssessmentQuestionCard({
  ...
  const options =
    (config.options as Array<{ label: string; score?: number; isCorrect?: boolean }>) ?? [];
  ...
  const correctMissing = hasOptions && !optionsMissing && !options.some((option) => option.isCorrect);
  ...
  const updateOptions = (
    nextOptions: Array<{ label: string; score?: number; isCorrect?: boolean }>,
  ) => {
    updateElement({ config: { options: nextOptions } });
  };

  const addOption = () => {
    updateOptions([...options, { label: "", score: 0, isCorrect: false }]);
  };
    ...
    <>
      <div className="relative flex gap-3 rounded-md border bg-white p-4 pr-14 shadow-sm">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          {(questionMissing || optionsMissing || correctMissing) && (
            <div className="space-y-1 text-sm text-red-500">
              ...
              {correctMissing && <p>Mark at least one correct answer</p>}
            </div>
          )}
        </div>
